...
# Assignment 3
# Invasion of the Blobs - Game for Python - Written by PyMike

# Modified by Evan Loughlin, Teale Masrani, and Joseph Gorespe
# Date: 2017.11.03

# This version of the the game utilizes the Analog Thumb Joystick by AdaFruit 
...
# This method converts movement from the Joystick to usable pixel value
def convert_movement(x):
        offset_correction = 2.384
        movement = (x - offset_correction)*3
        return movement

# This method returns whether the joystick is up, down, or at rest.
def discrete_movement(y):
        if y > 3:
            val = -1
        elif y < 2:
            val = 1
...
class Ship(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # Movement with joystick, with movement speed dependent upon joystick potentiometer reading.
        self.rect.move_ip(convert_movement(hat.analog.two.read()), 0)


        self.reload_timer += 1
        if (key[K_SPACE] or hat.input.one.read() > 0) and not self.overheated:
            self.heat += 0.75
...
class Game:
    ...
    def menuLoop(self):
        ...
        while 1:
            ...
            #Joystick Menu Input:
            y = hat.analog.one.read()
            newY = discrete_movement(y)

            if newY == 1:
                option = 1
            if newY == -1:
                option = 2
            if hat.input.one.read() > 0:
                if option == 1:
                    self.gameLoop()
                if option == 2:
                    pygame.quit()
